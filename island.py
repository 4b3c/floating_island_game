...
class island():
	...
	def to_classes(self):
		for tile, count in zip(self.top, range(len(self.top))):
			if tile[0] == "trees":
				self.top[count] = tile_classes.trees(tile[1])
			elif tile[0] == "rocks":
				self.top[count] = tile_classes.rocks(tile[1])
			elif tile[0] == "ruins":
				self.top[count] = tile_classes.ruins(tile[1])
			elif tile[0] == "grass":
	...
	def to_save(self):
		for tile, count in zip(self.top, range(len(self.top))):
			self.top[count] = [tile.name, tile.coords]
